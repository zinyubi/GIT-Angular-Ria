...
export function clamp01(v: any, fallback = 1) {
...
export function mergeStyle<T extends object | undefined>(
  base: T,
...
export function setWgsSource(mode: SrcName | "auto") {
  _forcedSrc = mode === "auto" ? null : mode;
}

let _normalizeWrapAround = false;
export function setWrapNormalization(enable: boolean) {
  _normalizeWrapAround = !!enable;
}
...
function getXYZ(p: any): [number, number, number] {
  ...
  if (!Number.isFinite(x) || !Number.isFinite(y)) {
    throw new Error(`Invalid point from transform`);
  }
  return [x, y, z];
}
function rtErr(aLon: number, aLat: number, bLon: number, bLat: number) {
  return Math.abs(aLon - bLon) + Math.abs(aLat - bLat);
}

const _srcPickCache = new WeakMap<CoordinateReference, SrcRefChoice>();
type TxKey = string;
type TxRec = {
  tx: ReturnType<typeof createTransformation>;
  srcName: SrcName;
};
const _txCache = new Map<TxKey, TxRec>();

/**
 * Build a cache key for a given source/target CRS pair.
 * Defensive: if src or trg are missing, we still return a stable key
 * instead of crashing on `.identifier`.
 */
function txKey(
  src: CoordinateReference | undefined,
  trg: CoordinateReference | undefined,
  wrap: boolean
): string {
  const s =
    src && (src as any).identifier
      ? String((src as any).identifier)
      : "src-unknown";
  const t =
    trg && (trg as any).identifier
      ? String((trg as any).identifier)
      : "trg-unknown";
...
function pickSourceRef(target: CoordinateReference): SrcRefChoice {
  ...
  const T_LON = 72.5714,
    T_LAT = 23.0225;
  let e84 = Number.POSITIVE_INFINITY,
    e4326 = Number.POSITIVE_INFINITY;

  try {
    const fwd = createTransformation(REF_CRS84, target, {
      normalizeWrapAround: _normalizeWrapAround,
    });
  ...
  try {
    const fwd = createTransformation(REF_EPSG4326, target, {
      normalizeWrapAround: _normalizeWrapAround,
    });
    const inv = fwd.inverseTransformation;
  ...
  const choice: SrcRefChoice =
    e84 <= e4326
      ? { src: REF_CRS84, name: "CRS:84" }
      : { src: REF_EPSG4326, name: "EPSG:4326" };
...
function getForwardTx(target: CoordinateReference): TxRec {
  if (!target) {
    throw new Error("getForwardTx: target CoordinateReference is required");
  }
  const { src, name } = pickSourceRef(target);
  const key = txKey(src, target, _normalizeWrapAround);
  let rec = _txCache.get(key);
  if (!rec) {
    const tx = createTransformation(src, target, {
      normalizeWrapAround: _normalizeWrapAround,
    });
    rec = { tx, srcName: name };
...
function srcWantsLatLon(srcName: SrcName) {
  return srcName === "EPSG:4326";
}
...
export function toMapPoint(
  ...
  return is3D
    ? createPoint(reference, [x, y, z])
    : createPoint(reference, [x, y]);
}
...
export function boundsFromLonLatRect(
  reference: CoordinateReference,
  minLon: number,
  minLat: number,
  maxLon: number,
  ...
  const wantsLatLon = srcWantsLatLon(srcName);
  const srcRef = wantsLatLon ? REF_EPSG4326 : REF_CRS84;
  const a = wantsLatLon ? [minLat, minLon] : [minLon, minLat];
  const b = wantsLatLon ? [maxLat, maxLon] : [maxLon, maxLat];
...
/* --------------------------- Debug --------------------------- */
export function debugLogRoundtrip(
  reference: CoordinateReference,
  lon: number,
  lat: number,
  tag = "dbg"
  ...
  try {
    ...
    console.warn(`[RiaViz:${tag}] roundtrip failed`, e);
  }
}
...
export function makeCircleCanvas(
  size: number,
  fill: string,
  stroke?: { color: string; width?: number }
) {
  const r = Math.max(1, size),
    pad = Math.max(1, stroke?.width ?? 0) + 1,
    w = r * 2 + pad * 2;
